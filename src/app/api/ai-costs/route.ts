...
const normalizeSummary = (value: unknown): CostSummary => {
  ...
  const openrouter = normalizeProvider(
    providers.openrouter ?? providers.open_router ?? providers["openrouter.ai"],
  );
  const elevenlabs = normalizeProvider(
    providers.elevenlabs ?? providers.eleven_labs ?? providers["elevenlabs.io"],
  );
  const fal = normalizeProvider(
    providers.fal ?? providers.falai ?? providers.fal_ai ?? providers["fal.ai"],
  );
  ...
  return {
    ...
    total_today:
      toNumber(root.total_today) ??
      sumKnown([openrouter.today, elevenlabs.today, fal.today]),
    total_month:
      toNumber(root.total_month) ??
      sumKnown([openrouter.month, elevenlabs.month, fal.month]),
  };
};
...
const fetchCostSummary = async (): Promise<CostSummary> => {
  ...
  if (!gatewayRaw || !token) {
    throw new Error(
      "AI Cost Center needs CLAW3D_GATEWAY_URL and CLAW3D_GATEWAY_TOKEN.",
    );
  }
  ...
  try {
    ...
    if (!content.trim()) {
      throw new Error("OpenClaw returned an empty AI costs response.");
    }
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const cacheSecondsRaw = Number(process.env.CLAW3D_COSTS_CACHE_SECONDS ?? "90");
    const cacheSeconds = Number.isFinite(cacheSecondsRaw)
      ? Math.max(30, Math.min(900, cacheSecondsRaw))
      : 90;
    cacheEntry = {
      data,
    ...
    const message =
      error instanceof Error ? error.message : "Failed to load AI costs.";
